...
export class Host {
    ...
    async start(web: Web, testing: boolean) {
    ...
    private async tryLocal(web: Web) {
    ...
    async localCheck(web: Web, urlDebug: string): Promise<boolean> {
        return await localCheck(web, urlDebug);
    }
}
...
function fetchLocalCheck(web: Web, url: string): Promise<any> {
    return new Promise((resolve, reject) => {
        web.fetch(url, fetchOptions as any)
            .then(v => {
...
async function localCheck(web: Web, url: string): Promise<boolean> {
    try {
        await fetchLocalCheck(web, url);
        return true;
    }
    catch (err) {
        return false;
